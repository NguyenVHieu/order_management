                ...
                foreach($orders as $order) {
                    ...
                    if ($product->merchize == null) {
                        ...
                        if ($product->merchize === 'All-over Print Kids and Youth Baseball Jersey Without Piping') {
                            ...
                            $type = in_array($order->style, ['Grommets 28x40 Double-sided grommet', 'Grommets 36x60 Double-sided grommet']) ? 'Vertical House' : 'Horizontal House';
        ...
        foreach($data as $keyParent => $orders) {
            ...
            try {
                ...
                foreach($orders as $key => $order) {
                    ...
                    if (empty($resSkuConvert['data']) || empty($order->color) || empty($order->size || ($product->private== null  && $order->is_tiktok == false))) {
    ...
    function pushOrderToGearment($data)
    {
        ...
        foreach($data as $key => $orders) {
            try {
                ...
                foreach($orders as $order) {
                    if (empty($order->shipping_method)) {
                        $shipping_method = $order->is_shipping == true ? 'METHOD_FAST_SHIP' : 'METHOD_STANDARD';
                    } else {
                        if (!in_array($order->shipping_method, ['METHOD_UNKNOWN', 'METHOD_ALL', 'METHOD_STANDARD', 'METHOD_GROUND', 'METHOD_FAST_SHIP', 'METHOD_STAMP'])) {
                            $results[][$order->order_number. ' '] = 'Phương thức vận chuyển không hợp lệ';
                            $check = false;
                            continue;   
                        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $data = [
                ...
                'place_order' => $request->place_order ?? null,
